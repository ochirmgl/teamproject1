...
class DesktopUITests(unittest.TestCase):
    ...
    def test_admin_pages_and_upload_validation(self):
        self.enter()
        for page in ("upload", "metadata", "users", "recycle", "processing", "chat", "library"):
    ...
    def test_docx_preview_has_readable_fallback_without_libreoffice(self):
        from docx import Document
        document = Document()
        document.add_heading('Test heading', level=1)
        document.add_paragraph('Readable fallback content')
        source = self.root / 'uploaded_files' / 'fallback.docx'
        document.save(source)
        with closing(database.open_database()) as conn, conn:
            doc_id = conn.execute("""INSERT INTO documents(title,file_path,file_type,status)
                VALUES ('Word','uploaded_files/fallback.docx','application/vnd.openxmlformats-officedocument.wordprocessingml.document','active')""").lastrowid
        self.enter()
        self.app.button(key=f'view_{doc_id}').click().run()
        self.assertTrue(any('Readable fallback content' in m.value for m in self.app.markdown))
        self.assert_clean()
