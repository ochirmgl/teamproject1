...
QUERY_STOPWORDS = {
    ...
    # Answer-format instructions must not become mandatory search concepts.
    "дэлгэрэнгүй", "тайлбарла", "тайлбарлаач", "өгөөч", "өгнө", "зөвхөн",
    "мэдээллийг", "мэдээлэл", "ашигла", "алхмуудыг", "алхмыг", "алхам",
    "байхгүй", "зохиож", "болохгүй", "яаж", "авах", "авна", "хэдэн",
    "detail", "detailed", "details", "steps", "step", "only", "please",
    "without", "invent", "provided", "source", "sources", "document", "documents",
}
...
class DocumentRAG:
    ...
    def retrieve(
        ...
        search_words = [word.lower() for word in WORD_PATTERN.findall(question + ' ' + expansion)
                        if word.lower() not in QUERY_STOPWORDS
                        and word.lower() not in ENGLISH_FOLLOW_UP_STOPWORDS]
        query_tokens = Counter(_tokenise(' '.join(search_words)))
        if not query_tokens:
            return []
        query_words = {
    ...
    def _is_summary_request(self, question: str) -> bool:
        ...
        return (any(pattern in normalised for pattern in patterns)
                or bool(re.search(r'\bwhat\s+(?:is|are)\b.+\babout\b', normalised))
                or 'юун тухай' in normalised)
    ...
    def _mentioned_document_ids(
        ...
        # Numbers refer to the persistent #ID displayed beside every document,
        # not its position in a sorted or filtered library.
        number = re.search(r'(?:#|\b(?:file|document)\s*#?\s*)(\d+)\b', question, re.I)
        if not number:
            number = re.search(r'\b(\d+)\s*(?:(?:дэх|дахь|дах|дүгээр|дугаар|th|st|nd|rd)\s*)?(?:файл|баримт|file|document)\b', question, re.I)
        if number:
            document_id = int(number.group(1))
            return [document_id] if document_id in selected and document_id in self.document_metadata else []
        question_words = {
    ...
    def _summary_sources(self, document_ids: Sequence[int]) -> list[RetrievedSource]:
        ...
        per_document_chars = max_total_chars // max(len(document_ids), 1)
        per_document_sources = max(1, 12 // max(len(document_ids), 1))
        ...
        for document_id in document_ids:
            ...
            if not metadata:
                continue
            # Pack short Word paragraphs, then sample across the entire document.
            # Previously twelve headings/paragraphs could consume every slot.
            units = []
            for page_label, section_text in self.document_sections.get(document_id, []):
                for piece in _split_text(section_text, max_chars=800, overlap=0):
                    if units and len(units[-1][1]) + len(piece) + 1 <= 800:
                        old_label, old_text = units[-1]
                        label = old_label if page_label in old_label else old_label + '; ' + page_label
                        units[-1] = (label, old_text + ' ' + piece)
                    else:
                        units.append((page_label, piece))
            slots = min(per_document_sources, len(units))
            positions = ([round(i * (len(units)-1) / (slots-1)) for i in range(slots)]
                         if slots > 1 else [0] if slots else [])
            for position in positions:
                page_label, section_text = units[position]
                if document_chars >= per_document_chars or document_sources >= per_document_sources:
                    break
                remaining = min(per_document_chars - document_chars,
                                per_document_chars // max(slots, 1))
                ...
                document_chars += len(context_text)
                document_sources += 1
        return sources
