...
def migrate_database(path=None):
    ...
    try:
        ...
        if version >= 6 and 'selection_initialized' in columns:
            return
        if version > 6:
            raise RuntimeError('Database schema is newer than this application; deploy matching code.')
        ...
        if 'selection_initialized' not in {r[1] for r in conn.execute('PRAGMA table_info(chat_sessions)')}:
            conn.execute('ALTER TABLE chat_sessions ADD COLUMN selection_initialized INTEGER NOT NULL DEFAULT 0')
            conn.execute('UPDATE chat_sessions SET selection_initialized=1 WHERE id IN (SELECT session_id FROM chat_session_documents)')
