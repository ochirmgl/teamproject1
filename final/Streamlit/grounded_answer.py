...
def answer(index, question, selected, history, config, user_id, db_path):
    ...
    if index._is_summary_request(question):
...
Keep the answer concise, at most 12 claims. For a multi-document summary, cover each supplied document separately. Summaries describe only the supplied excerpts and say so.
