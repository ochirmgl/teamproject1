...
def view_document_dialog(doc_title, file_path, file_type):
    ...
    # Эх Word файлыг LibreOffice -> PDF болгон хөрвүүлээд яг хуудасны байдлаар харуулна.
        ...
        try:
            ...
            if ext == ".docx":
                try:
                    status, message, sections = processing.extract(resolved_path)
                    if sections:
                        st.info("Word-ийн хуудасны дүрслэлд LibreOffice шаардлагатай. Доор уншигдах агуулгыг харуулав.")
                        if message:
                            st.warning(message)
                    ...
                    st.error(f"Word файлыг харахад алдаа гарлаа: {fallback_error}")
            else:
                st.error(f"Хуучин DOC файлын дүрслэлд LibreOffice шаардлагатай: {e}")
                st.info("DOCX болгон хадгалаад дахин оруулбал LibreOffice-гүйгээр агуулгыг уншиж болно.")
