...
def get_config_value(name, default=None):
    """Explicit deployment Secrets override stale local/environment values."""
    try:
...
# Fail clearly when a manual GitHub upload mixes different application versions.
def deployment_compatibility_errors():
    ...
    try:
        ...
        conn.close()
    if 'selection_initialized' not in columns:
        # A deployment can restore an older DB while the resource cache still
        # says initialization ran. Repair actual schema, not the cached result.
        # init_db uses the existing backup/migration path; never replace the DB.
        init_db(str(DB_PATH))
        conn = open_database()
        try:
            columns = {row[1] for row in conn.execute('PRAGMA table_info(chat_sessions)')}
            ...
            conn.close()
    if 'selection_initialized' not in columns:
        missing.append('database.py: chat_sessions.selection_initialized багана үүсээгүй.')
    if 'user_daily_limit' not in inspect.signature(AIConfig).parameters:
        missing.append('ai_provider.py: AIConfig.user_daily_limit тохиргоо байхгүй.')
    return missing


compatibility_errors = deployment_compatibility_errors()
if compatibility_errors:
    st.error('Шинэчлэлт дутуу байна. App болон туслах файлуудын хувилбар таарахгүй байна.')
    for problem in compatibility_errors:
        st.write(problem)
    st.info('GitHub дахь ажиллаж буй app.py-тай нэг хавтсанд шинэ database.py болон ai_provider.py файлыг солиод Streamlit-ийг Reboot хийнэ үү. Өгөгдлийн сан болон uploaded_files-ийг бүү солиорой.')
    st.stop()
...
def create_chat_session(user_id):
    conn = open_database()
    cursor = conn.cursor()
    # Reuse an unfinished draft instead of adding another empty sidebar entry.
    cursor.execute("""SELECT s.id FROM chat_sessions s WHERE s.user_id=?
...
def list_chat_sessions(user_id):
    ...
    cursor.execute(
        """SELECT s.id, s.title, s.updated_at, COUNT(m.id),
           (SELECT content FROM chat_messages first_message
            WHERE first_message.session_id=s.id AND first_message.role='user'
            ORDER BY first_message.id LIMIT 1)
    ...
    return [
        {
            "id": row[0],
            "title": (" ".join((row[4] or '').split())[:60] or 'Шинэ чат') if row[1] == 'Шинэ чат' else row[1],
            "updated_at": row[2],
            "message_count": row[3],
        }
        for row in rows
...
def save_chat_exchange(session_id, user_id, question, answer, sources=None):
    ...
    if session_row[0].lower().strip(' !?.') in {"шинэ чат", "hi", "hello", "hey", "сайн уу", "сайн байна уу"}:
...
def load_chat_document_ids(session_id, user_id):
    conn = open_database()
    initialized = conn.execute('SELECT selection_initialized FROM chat_sessions WHERE id=? AND user_id=?', (session_id,user_id)).fetchone()
    if initialized and not initialized[0]:
        conn.close()
        return None
    cursor = conn.cursor()
    cursor.execute(
...
def save_chat_document_ids(session_id, user_id, document_ids):
    ...
    cursor.execute('UPDATE chat_sessions SET selection_initialized=1 WHERE id=? AND user_id=?',(session_id,user_id))
    conn.commit()
    conn.close()
    ...
    with st.expander(f"📚 Ашигласан эх сурвалж ({len(sources)})"):
        for source in sources:
            ...
            if source.get('file_path'):
                conn = open_database()
                allowed = conn.execute("""SELECT 1 FROM documents d WHERE d.id=? AND d.status='active'
                    AND (d.file_path=? OR EXISTS (SELECT 1 FROM document_versions v
                    WHERE v.document_id=d.id AND v.file_path=?))""",
                    (source['document_id'],source['file_path'],source['file_path'])).fetchone()
                conn.close()
                if allowed and local_file_path(source['file_path']).is_file() and st.button('Эх хуудас нээх',key=f"source_{key_prefix}_{source['number']}"):
                    import re
                ...
                elif not allowed or not local_file_path(source['file_path']).is_file():
                    st.caption('Энэ эх хувилбар одоо боломжгүй байна. Баримтын сангаас шинэ хувилбарыг шалгана уу.')
...
def answer_system_question(question, selected_document_ids, documents):
    ...
    count_patterns = (
        "how many file", "how many document", "number of files", "хэдэн файл",
        "хэдэн баримт", "файлын тоо", "баримтын тоо"
    )
    if any(pattern in normalised for pattern in count_patterns):
        if not re.search(r'[А-Яа-яӨөҮүЁё]', normalised):
            return f"There are {len(selected)} documents selected for this AI chat."
    ...
    if any(pattern in normalised for pattern in list_patterns):
        ...
        names = "\n".join(
            f"#{document['id']}: {document['title']}" for document in selected
        )
...
def view_document_dialog(doc_title, file_path, file_type, initial_page=1):
    ...
    st.download_button("Эх файл татах", resolved_path.read_bytes(),
                       file_name=original_filename(file_path), key="viewer_original_download")
    try:
...
def original_filename(stored):
    conn = open_database()
    try:
        row = conn.execute('SELECT original_name FROM document_versions WHERE file_path=? AND original_name IS NOT NULL ORDER BY id DESC LIMIT 1',(stored,)).fetchone()
        return row[0] if row and row[0] else Path(str(stored)).name
    finally:
        conn.close()
...
def library(is_admin):
    ...
    filter_signature = (term,kind,category,tag,author,order,len(filtered))
    if st.session_state.get('library_page_filters') != filter_signature:
        st.session_state.library_page_filters = filter_signature
        st.session_state.library_page = 1
    page_count = max(1,(len(filtered)+11)//12)
    page_number = st.selectbox('Хуудас', list(range(1,page_count+1)),key='library_page') if page_count > 1 else 1
    visible_documents = filtered[(page_number-1)*12:page_number*12]
    for offset in range(0, len(visible_documents), 3):
        for column, doc in zip(st.columns(3), visible_documents[offset:offset + 3]):
            with column, st.container(border=True, key=f"document_card_{doc[0]}"):
                ...
                with more:
                    with st.popover("Үйлдлүүд", use_container_width=True):
                        ...
                        if path.exists() and st.button('Татахад бэлтгэх',key=f'prepare_download_{doc[0]}'):
                            st.download_button("Эх файл татах", path.read_bytes(), file_name=original_filename(doc[3]), mime=doc[4], key=f"download_{doc[0]}", use_container_width=True)
...
def ai_configuration():
    ...
    return AIConfig(provider=provider,
        ...
        api_key=str(get_config_value('GROQ_API_KEY' if provider == 'groq' else 'GEMINI_API_KEY','')).strip(),
        ...
        user_daily_limit=integer_setting('AI_USER_DAILY_REQUEST_LIMIT',10,1000),
...
def ai_settings_page():
    ...
    key_setting = 'GROQ_API_KEY' if config.provider == 'groq' else 'GEMINI_API_KEY'
    try:
        key_source = 'Streamlit Secrets' if key_setting in st.secrets else 'Environment / .env'
    except (FileNotFoundError, KeyError):
        key_source = 'Environment / .env'
    st.caption(f'Түлхүүрийн эх үүсвэр: {key_source}. Түлхүүр тохируулсан: {bool(config.api_key)}. Үнэгүй төлөв баталгаажсан: {config.free_tier_confirmed}.')
    ...
    if not rows:
        st.info('Өнөөдөр гадаад AI үйлчилгээ рүү шинэ хүсэлт бүртгэгдээгүй. Дотоод хайлтын хариу болон хадгалсан хариу энд тоологдохгүй.')
...
if st.session_state.logged_in:
    ...
    if page == "library":
        ...
        saved_selection = load_chat_document_ids(active_session_id, user_id)
        saved_document_ids = [
            document_id
            for document_id in (saved_selection or [])
            if document_id in document_map
        ]
        default_document_ids = list(document_map) if saved_selection is None else saved_document_ids
        selection_key = f"chat_documents_{active_session_id}"
        if selection_key not in st.session_state:
            st.session_state[selection_key] = default_document_ids
        st.session_state[selection_key] = [v for v in st.session_state[selection_key] if v in document_map]

        with st.expander(
            f"📚 Асуулт асуух баримтууд ({len(st.session_state[selection_key])}/{len(document_map)})",
            expanded=False,
        ):
            st.caption(
            ...
            selected_ids = st.multiselect(
                ...
                format_func=lambda document_id: f"#{document_id}: {document_map[document_id]}",
                placeholder="Нэг эсвэл хэд хэдэн баримт сонгоно уу",
                key=f"chat_documents_{active_session_id}",
                label_visibility="collapsed",
            )
            st.caption('Дугаар нь баримтын тогтмол #ID. Жишээ: «#10 баримтыг товчлооч». Шинэ файл автоматаар сонгогдохгүй.')
            def set_selection(values):
                st.session_state[selection_key] = values
            left, right = st.columns(2)
            left.button('Бүгдийг сонгох',key='select_all_chat_docs',on_click=set_selection,args=(list(document_map),))
            right.button('Сонголт цэвэрлэх',key='clear_chat_docs',on_click=set_selection,args=([],))
        save_chat_document_ids(active_session_id, user_id, selected_ids)
        if len(selected_ids) < len(document_map):
            st.info(f'{len(document_map) - len(selected_ids)} баримт энэ чатад сонгогдоогүй байна. '
                    'Админ болон хэрэглэгч ижил идэвхтэй баримтын сантай; чат бүрийн сонголт тусдаа хадгалагдана.')
            st.button('Бүх баримтаас асуух', key='use_all_chat_docs', on_click=set_selection, args=(list(document_map),))
        ...
        chat_requests.ensure_schema(DB_PATH)
        pending_request = chat_requests.recover(DB_PATH, active_session_id, user_id)
        if pending_request:
            st.info('Өмнөх хүсэлт боловсруулагдаж байна. Тасарсан хүсэлт 10 минутын дараа дахин шалгахад сэргээгдэнэ.')
            if st.button('Хүсэлтийн төлөв шинэчлэх'):
                st.rerun()
        chat_disabled = pending_request or not provider_ready or rag_index.document_count == 0 or not selected_ids
        ...
        if prompt:
            request_id = st.session_state.setdefault('chat_submission_id', str(uuid.uuid4()))
            if not chat_requests.begin(DB_PATH, request_id, active_session_id, user_id, prompt):
                st.session_state.pop('chat_submission_id', None)
                st.rerun()
            ...
            with st.chat_message("assistant"):
                try:
                    ...
                    st.caption({'local':'Дотоод хариулт · API хүсэлт илгээгээгүй', 'cache':'Хадгалсан AI хариулт · шинэ API хүсэлтгүй', 'api':'AI үйлчилгээний шинэ хүсэлт'}.get(config.last_response_kind,'Дотоод хариулт'))
                    ...
                    request_failed = True
                    response = 'Хүсэлтийг гүйцэтгэж чадсангүй. Асуулт хадгалагдсан. Дахин оролдоно уу.'
                    st.error(response)

            chat_requests.finish(DB_PATH, request_id, user_id, response, source_payload, failed=request_failed)
            st.session_state.pop('chat_submission_id', None)
            ...
            # All source buttons must have the same history keys on their next click.
            # Previously "current" became "history_N", losing the click on rerun.
            st.rerun()
