...
def ai_configuration():
    ...
    provider = get_config_value('AI_PROVIDER','gemini')
    ...
    return AIConfig(provider=provider,
        model=get_config_value('OLLAMA_MODEL','qwen3:4b') if provider == 'ollama' else get_config_value('GEMINI_MODEL','gemini-2.5-flash'),
        api_key=get_config_value('GEMINI_API_KEY',''),
        free_tier_confirmed=str(get_config_value('GEMINI_FREE_TIER_CONFIRMED','false')).lower() == 'true',
...
def ai_settings_page():
    ...
    st.info('Gemini ашиглах бол billing холбогдоогүй үнэгүй төсөл ашиглана. Апп түлхүүрээс billing төлөвийг тодорхойлох боломжгүй.')
    conn = open_database()
...
if st.session_state.logged_in:
    ...
    if page == "library":
        ...
        config = ai_configuration()
        provider_ready = config.provider == 'ollama' or (config.provider == 'gemini' and config.api_key and config.free_tier_confirmed)
