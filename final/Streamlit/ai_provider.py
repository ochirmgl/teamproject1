...
def invalidate_answer(config, prompt, user_id, path=None):
    """Keep accounting, but never reuse a response rejected by source validation."""
    ...
    with closing(open_database(path)) as conn, conn:
        conn.execute("UPDATE ai_requests SET status='invalid_evidence' WHERE cache_key=? AND user_id=? AND status='complete'", (key,user_id))
